...
namespace cartesian
variable (E:cartesian α )

def to_vector (E:cartesian α):  E.carrier  ≃ vector α (E.dim) := equiv.trans (E.equiv) (@fun_vector α _ E.dim) 

instance : has_coe_to_sort (cartesian α)  := ⟨_, @cartesian.carrier α⟩
instance to_topological_space (E : cartesian α  ) :   topological_space E := induced (to_vector E) (by apply_instance)

def to_Top (E : cartesian α  ) : Top :=⟨E, by apply_instance⟩
instance : has_coe (cartesian α)  Top :=⟨to_Top⟩

def cartesian_space (n : ℕ) : cartesian α  :=⟨vector α  n, n, (fun_vector n).symm ⟩
notation `α^` := cartesian_space

instance   : add_comm_group (E:cartesian α) := equiv.add_comm_group E.equiv
...
def dual:cartesian α  := ⟨(E →ₗ[α] α), E.dim, (@dual_equiv_std α _ E).to_equiv ⟩ 

end dual

def field (α) [normed_field α] : cartesian α  :=⟨α , 1, fun_one_equiv.symm⟩

def proj (E:cartesian α ) (i:fin (E.dim)) : E →ₗ[α] α  := (linear_map.proj i).comp ((equiv_vec E).to_linear_map) 

def std_basis (E:cartesian α) (i:fin E.dim): E := E.equiv.inv_fun (@euclidean.std_basis α E.dim _ i) 

instance has_norm : has_norm (E:cartesian α) :=⟨λ  x,  ∥E.equiv x∥⟩

instance : has_dist (E:cartesian α )  := ⟨ λ x y, @has_dist.dist _ _ (E.equiv x) (E.equiv y)   ⟩ 

 instance metric_space  :metric_space (E:cartesian α ) :=
{ metric_space.
  dist_self:= by {intro, simp[dist], 
    have h:= (@euclidean.normed_space α E.dim _).dist_self,
    have h1:=@h (E.equiv x),
    simp [dist] at h1, apply h1  },
